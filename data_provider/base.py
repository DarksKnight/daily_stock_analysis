...
class BaseFetcher(ABC):
    ...
    def get_sector_limit_stats(self, trade_date: Optional[str] = None) -> Optional[List[Dict]]:
        ...
        Args:
            trade_date: 日期字符串 YYYYMMDD，默认今日

        Returns:
            List[Dict]，每项含: name, change_pct, limit_up_count, limit_down_count,
                               up_count, down_count
        """
        return None
...
class DataFetcherManager:
    ...
    def _prune_fundamental_cache(self, ttl_seconds: int, max_entries: int) -> None:
        ...
        with self._fundamental_cache_lock:
            ...
            if ttl_seconds > 0:
                ...
                expired_keys = [key for key, value in cache_items if now_ts - float(value.get("ts", 0)) > ttl_seconds]
    ...
    def _normalize_belong_boards(raw_data: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(raw_data, pd.DataFrame):
            ...
            code_col = next(
                (col for col in raw_data.columns if str(col) in {"板块代码", "代码", "code"}),
                None,
            )
            type_col = next(
                (col for col in raw_data.columns if str(col) in {"板块类型", "类别", "type"}),
                None,
            )
            if name_col is None:
                return []
        ...
        if isinstance(raw_data, (list, tuple, set)):
            for item in raw_data:
                if isinstance(item, dict):
                    ...
                    code_raw = item.get("code") or item.get("板块代码") or item.get("代码")
                    if not DataFetcherManager._is_missing_board_value(code_raw):
                        normalized_item["code"] = str(code_raw).strip()
                    type_raw = item.get("type") or item.get("板块类型") or item.get("类别")
    ...
    def get_fundamental_context(self, stock_code: str, budget_seconds: Optional[float] = None) -> Dict[str, Any]:
        ...
        bundle_chain = (
            ...
            else self._normalize_source_chain(
                None,
                "fundamental_bundle",
                bundle_status,
                bundle_ms,
    ...
    def get_sector_limit_stats(self, trade_date: Optional[str] = None) -> List[Dict]:
        """获取板块涨停/跌停家数统计（自动切换数据源）"""
        for fetcher in self._fetchers:
            if not hasattr(fetcher, "get_sector_limit_stats"):
                continue
            try:
                data = fetcher.get_sector_limit_stats(trade_date=trade_date)
                if data:
                    logger.info("[%s] 获取板块涨跌停统计成功: %d 条", fetcher.name, len(data))
                    return data
            except Exception as e:
