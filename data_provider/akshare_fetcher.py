...
class AkshareFetcher(BaseFetcher):
    ...
    def get_sector_limit_stats(self, trade_date: Optional[str] = None) -> Optional[List[Dict]]:
        ...
        Returns:
            List[Dict]，每项含:
              name, change_pct, limit_up_count, limit_down_count,
              up_count, down_count
        """
        import akshare as ak
        from datetime import datetime as _dt

        date_str = trade_date or _dt.now().strftime("%Y%m%d")
        ...
        try:
            self._set_random_user_agent()
            self._enforce_rate_limit()
            logger.info("[API调用] ak.stock_zt_pool_em(date=%s) 获取涨停股票池...", date_str)
            df_zt = ak.stock_zt_pool_em(date=date_str)
            if df_zt is not None and not df_zt.empty and "所属行业" in df_zt.columns:
                for sector in df_zt["所属行业"].dropna():
                    sector = str(sector).strip()
                    if sector:
                        limit_up_by_sector[sector] = limit_up_by_sector.get(sector, 0) + 1
                logger.info("[Akshare] 涨停池 %d 只，覆盖 %d 个行业", len(df_zt), len(limit_up_by_sector))
        except Exception as e:
            logger.warning("[Akshare] 获取涨停股票池失败: %s", e)
        ...
        try:
            ...
            if df_dt is not None and not df_dt.empty and "所属行业" in df_dt.columns:
                for sector in df_dt["所属行业"].dropna():
                    sector = str(sector).strip()
                    if sector:
                        limit_down_by_sector[sector] = limit_down_by_sector.get(sector, 0) + 1
                logger.info("[Akshare] 跌停池 %d 只，覆盖 %d 个行业", len(df_dt), len(limit_down_by_sector))
        except Exception as e:
            logger.warning("[Akshare] 获取跌停股票池失败: %s", e)

        # 如果两个涨跌停池都为空，认为接口暂无数据（盘中或非交易日）
        if not limit_up_by_sector and not limit_down_by_sector:
            logger.warning("[Akshare] 涨停/跌停股票池均为空，跳过板块涨跌停统计")
            return None

        # 3. 行业板块行情 → 获取涨跌幅、上涨/下跌家数
        sector_base: Dict[str, Dict] = {}
        try:
            self._set_random_user_agent()
            self._enforce_rate_limit()
            logger.info("[API调用] ak.stock_board_industry_name_em() 获取板块行情...")
            ...
            logger.warning("[Akshare] 获取板块行情失败，涨跌幅/家数将为 0: %s", e)

        # 4. 合并：以涨停/跌停池覆盖的行业 + 行业行情表 为全集
        all_sector_names = set(limit_up_by_sector) | set(limit_down_by_sector) | set(sector_base)
        result = []
        for name in all_sector_names:
            base = sector_base.get(name, {})
            result.append(
                {
                    "name": name,
                    "change_pct": base.get("change_pct", 0.0),
                    "limit_up_count": limit_up_by_sector.get(name, 0),
                    "limit_down_count": limit_down_by_sector.get(name, 0),
                    "up_count": base.get("up_count", 0),
                    "down_count": base.get("down_count", 0),
                }
            )

        result.sort(key=lambda x: -x["limit_up_count"])
        logger.info("[Akshare] 板块涨跌停统计完成: %d 个板块", len(result))
