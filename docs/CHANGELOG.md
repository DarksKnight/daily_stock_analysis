...
- 🔥 **大盘复盘热点解读：板块涨停/跌停家数排行** — 大盘复盘（A 股）现可获取当日前 10 领涨板块（涨停家数）和领跌板块（跌停数量），数据来源于东财涨停/跌停股票池（`ak.stock_zt_pool_em` / `ak.stock_dt_pool_em`），按板块聚合后与行业行情合并；结果注入复盘报告"热点解读"章节（含涨幅榜和停板家数两张表），同时持久化到新建的 `sector_limit_stats` 数据库表，可供后续选股查询。
