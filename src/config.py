...
def normalize_llm_channel_model(model: str, protocol: Optional[str], base_url: Optional[str] = None) -> str:
    ...
    if "/" in normalized_model:
        ...
        known_providers = (
            ...
            | set(SUPPORTED_LLM_CHANNEL_PROTOCOLS)
            | {
                "cohere",
                "huggingface",
                "bedrock",
...
def get_effective_agent_primary_model(config: "Config") -> str:
    ...
    configured_router_models = set(get_configured_llm_models(getattr(config, "llm_model_list", []) or []))
...
def get_effective_agent_models_to_try(config: "Config") -> List[str]:
    ...
    configured_router_models = set(get_configured_llm_models(getattr(config, "llm_model_list", []) or []))
    raw_models = [get_effective_agent_primary_model(config)] + (getattr(config, "litellm_fallback_models", []) or [])
    seen = set()
...
class Config:
    ...
    # 东财选股用户标识（qgqp_b_id Cookie），用于智能选股接口鉴权
    eastmoney_qgqp_b_id: Optional[str] = None
    # 实时行情数据源优先级（逗号分隔）
    ...
    def __post_init__(self) -> None:
        ...
        if self.agent_skill_routing not in self._VALID_SKILL_ROUTING:
            _log.warning(
                ...
                self.agent_skill_routing,
                self._VALID_SKILL_ROUTING,
            )
    ...
    def _load_from_env(cls) -> "Config":
        ...
        return cls(
            ...
            gemini_max_retries=parse_env_int(
                os.getenv("GEMINI_MAX_RETRIES"), 5, field_name="GEMINI_MAX_RETRIES", minimum=0
            ),
            gemini_retry_delay=parse_env_float(
                os.getenv("GEMINI_RETRY_DELAY"), 5.0, field_name="GEMINI_RETRY_DELAY", minimum=0.0
            ),
            anthropic_api_key=os.getenv("ANTHROPIC_API_KEY"),
            anthropic_model=os.getenv("ANTHROPIC_MODEL", "claude-3-5-sonnet-20241022"),
            anthropic_temperature=parse_env_float(
                os.getenv("ANTHROPIC_TEMPERATURE"), 0.7, field_name="ANTHROPIC_TEMPERATURE"
            ),
            anthropic_max_tokens=parse_env_int(
                os.getenv("ANTHROPIC_MAX_TOKENS"), 8192, field_name="ANTHROPIC_MAX_TOKENS", minimum=1
            ),
            ...
            agent_skill_autoweight=(
                os.getenv("AGENT_SKILL_AUTOWEIGHT") or os.getenv("AGENT_STRATEGY_AUTOWEIGHT", "true")
            ).lower()
            == "true",
            agent_skill_routing=(
                os.getenv("AGENT_SKILL_ROUTING") or os.getenv("AGENT_STRATEGY_ROUTING", "auto")
            ).lower(),
            ...
            feishu_max_bytes=parse_env_int(
                os.getenv("FEISHU_MAX_BYTES"), 20000, field_name="FEISHU_MAX_BYTES", minimum=1
            ),
            wechat_max_bytes=wechat_max_bytes,
            wechat_msg_type=wechat_msg_type_lower,
            discord_max_words=parse_env_int(
                os.getenv("DISCORD_MAX_WORDS"), 2000, field_name="DISCORD_MAX_WORDS", minimum=1
            ),
            ...
            backtest_eval_window_days=parse_env_int(
                os.getenv("BACKTEST_EVAL_WINDOW_DAYS"), 10, field_name="BACKTEST_EVAL_WINDOW_DAYS", minimum=1
            ),
            backtest_min_age_days=parse_env_int(
                os.getenv("BACKTEST_MIN_AGE_DAYS"), 14, field_name="BACKTEST_MIN_AGE_DAYS", minimum=1
            ),
            ...
            # 东财选股用户标识
            eastmoney_qgqp_b_id=os.getenv("EASTMONEY_QGQP_B_ID") or None,
            # 实时行情数据源优先级：
            ...
            circuit_breaker_cooldown=parse_env_int(
                os.getenv("CIRCUIT_BREAKER_COOLDOWN"), 300, field_name="CIRCUIT_BREAKER_COOLDOWN", minimum=0
            ),
    ...
    def _parse_llm_channels(cls, channels_str: str) -> List[Dict[str, Any]]:
        ...
        for raw_name in channels_str.split(","):
            ...
            protocol = resolve_llm_channel_protocol(
                protocol_raw, base_url=base_url, models=raw_models, channel_name=ch_name
            )
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        if available_router_model_set:
            ...
            invalid_fallbacks = [
                model
                for model in (self.litellm_fallback_models or [])
            ...
            issues.append(
                ConfigIssue(
                    severity="error",
                    message=("AGENT_LITELLM_MODEL 已配置，但未找到可用的运行时来源" "（启用渠道或匹配的 API Key）。"),
                    field="AGENT_LITELLM_MODEL",
                )
            )
