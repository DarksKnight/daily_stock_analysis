...
try:
    ...
    _logging.getLogger(__name__).warning("EFINANCE_CALL_TIMEOUT is not a valid integer, using default 30s")
    _EF_CALL_TIMEOUT = 30
...
_realtime_cache: Dict[str, Any] = {"data": None, "timestamp": 0, "ttl": 600}  # 10分钟缓存有效期

# ETF 实时行情缓存（与股票分开缓存）
_etf_realtime_cache: Dict[str, Any] = {"data": None, "timestamp": 0, "ttl": 600}  # 10分钟缓存有效期
